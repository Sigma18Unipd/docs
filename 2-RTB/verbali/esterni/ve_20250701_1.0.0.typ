...
Il seguente documento contiene il resoconto dell'incontro con l'azienda proponente, _Var Group S.p.A._, tenutosi in modalità *remota* il *1 Luglio 2025* e della durata di *30 minuti*.
...
#pagebreak()
= Ordine del giorno
Durante l'incontro svolto con l'azienda, il gruppo _Sigma18_ ha mostrato il #glossario("PoC (Proof of Concept)") sviluppato e in particolare si sono trattati i seguenti punti:

== Presentazione del PoC
Il gruppo ha presentato il _PoC_ sviluppato, mostrando le funzionalità finora implementate e il funzionamento generale del sistema.\
Sono stati mostrati i seguenti aspetti:
...
Il gruppo, infine, ha richiesto la modifica di un requisito funzionale. In particolare, si è discusso del requisito _ROF-18_, il quale prevede che l'utente possa aggiungere un blocco a una #glossario("routine") esistente. Tale requisito era stato inizialmente pensato per essere implementato tramite l'utilizzo del #glossario("drag & drop"), ma successivamente il gruppo si è accordato con il proponente per adottare un approccio modale: un pulsante permetterà di aggiungere un blocco alla routine, dopodiché l'utente potrà collegare e modificare i blocchi a proprio piacimento.
